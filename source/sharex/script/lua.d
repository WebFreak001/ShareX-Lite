...
private string handleJob(UploadJob* job)
{
	...
	while(!isDone) { Thread.sleep(50.msecs); }
	if(!evnt.success)
    ...
    void run(string file)
    {
        ...
        new Thread({
            try
            {
                ...
                lua["uploadImage"] = (size_t ptr) { if(ptr == 0) throw new Exception("Bitmap null"); return handleJob(uploadImage(addJob, data, *(cast(Bitmap*)ptr))); };
                ...
                lua["saveImage"] = (size_t ptr) { if(ptr == 0) throw new Exception("Bitmap null"); return saveImage(*(cast(Bitmap*)ptr)); };

                lua["copyText"] = &Clipboard.setText;
                lua["copyImage"] = (size_t ptr) { if(ptr == 0) return; Clipboard.setImage(*(cast(Bitmap*)ptr)); };
